...
class CustomsCheckApp(QMainWindow):
    ...
    def check_tedarikci_beyan_kontrol(self):
        ...
        try:
            ...
            # Tedarikçi beyan kontrolü için gerekli sütunları kontrol et
            required_columns = ['Tedarikci_adi', 'Tedarikci_firma', 'Tedarikci_ulke', 'Tedarikci_vergi_no']
            missing_columns = [col for col in required_columns if col not in self.current_df.columns]
            ...
            # Tedarikçi beyan kontrolü için gerekli sütunların varlığını kontrol et
            if all(col in self.current_df.columns for col in required_columns):
                # Tedarikçi beyan kontrolü için gerekli sütunların doğruluğunu kontrol et
                if self.current_df['Tedarikci_vergi_no'].notna().all():
                    self.status_label.setText("Tedarikçi Beyan kontrolü: Tüm veriler doğru ve tamamlanmıştır.")
                    self.progress_bar.setVisible(False)
                else:
                    QMessageBox.warning(self, "Uyarı", "Tedarikçi vergi numarası eksik veya boş bırakılmıştır.")
                    self.progress_bar.setVisible(False)
            else:
                QMessageBox.warning(self, "Uyarı", "Tedarikçi beyan kontrolü için gerekli sütunlar eksik veya boş bırakılmıştır.")
                self.progress_bar.setVisible(False)
    ...
    def _generate_tedarikci_beyan_html_report(self, check_result):
        """Tedarikçi beyan kontrolü için HTML raporu oluştur"""
        html_content = f"""
        <html>
        <head>
            <title>Tedarikçi Beyan Kontrol Raporu</title>
            <style>
        ...
        <body>
            <h1>Tedarikçi Beyan Kontrol Raporu</h1>
            <div class="summary">
